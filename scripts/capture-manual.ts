...
async function captureDefinition(page: Page, def: ScreenshotDefinition, manifest: Manifest): Promise<void> {
  ...
  try {
    ...
    if (def.optional) {
      console.warn(`[capture] ${def.id}: SKIPPED — page did not render its content at ${url} (${(err as Error).message})`);
      return;
    }
    throw err;
  }
...
async function run(parsed: ParsedArgs): Promise<void> {
  ...
  if (manifest.length > 30) {
    throw new Error(`PNG budget violated: ${manifest.length} screenshots exceed the 30-screenshot budget`);
  }
